import { Injectable } from '@angular/core';
import { CompanyInfo } from '../misc/models';

@Injectable({
  providedIn: 'root'
})
export class CompanyService {
  companies: CompanyInfo[] = [
    {
      name: "Kompanija 1",
      area: CompanyInfo.areas[0],
      city: "Beograd",
      picture: "data:image/png;base64,"
    },
    {
      name: "Proba 123",
      area: CompanyInfo.areas[2],
      city: "Novi sad",
      picture: "data:image/png;base64,"
    },
    {
      name: "Blabla",
      area: CompanyInfo.areas[1],
      city: "Kragujevac",
      picture: "data:image/png;base64,"
    }
  ];
  constructor() { }

  async search(name: String, city: String, areas: String[]): Promise<CompanyInfo[]> {
    var filtered: CompanyInfo[] = [];
    this.companies.forEach((comp) => { filtered.push(comp) });
    var newFiltered: CompanyInfo[] = [];
    if (name) {
      name = name.toUpperCase();
      filtered.forEach((c) => { if (c.name.toUpperCase().indexOf(name.toString()) != -1) newFiltered.push(c) })
      filtered = newFiltered;
      newFiltered = [];
    }
    if (city) {
      city = city.toUpperCase();
      filtered.forEach((c) => { if (c.city.toUpperCase().indexOf(city.toString()) != -1) newFiltered.push(c) })
      filtered = newFiltered;
      newFiltered = [];
    }
    if (areas && areas.length>0) {
      areas.forEach((area) => {
        filtered.forEach((c) => {
          if (c.area == area) newFiltered.push(c);
        })
      });
      filtered = newFiltered;
      newFiltered = [];
    }
    return filtered;
  }
}
